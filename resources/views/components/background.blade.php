...
@if($styles)
    @php
        ...
        $width = (int)round($imageWidth/100*$scale);
        $height = (int)round($imageHeight/100*$scale);
        $left = collect([random_int(-((int)($width/4)), 0).'px', random_int(-25, 25).'vw'])->random();
        $top = collect([random_int(-((int)($height/3)), (int)($height/3)).'px', random_int(0, 15).'vh'])->random();
